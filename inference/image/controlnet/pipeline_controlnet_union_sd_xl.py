...
from image.controlnet.controlnet_union import ControlNetModel_Union
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # NOTE:
                # `control_type` 的 batch 维必须与传入 ControlNet 的 sample batch 对齐。
                # 当 guidance_scale <= 1（do_classifier_free_guidance=False）时，latent_model_input 不会 *2，
                # 但这里历史实现仍固定 repeat(...*2)，会导致 control_type 的 batch=2 而 sample batch=1，
                # 最终在 control_add_embedding 里触发 matmul 维度错误（例如 3072 vs 1536）。
                repeat_factor = batch_size * num_images_per_prompt * (2 if do_classifier_free_guidance else 1)
                added_cond_kwargs = {
                    "text_embeds": add_text_embeds,
                    "time_ids": add_time_ids,
                    "control_type": union_control_type.reshape(1, -1)
                    .to(device, dtype=prompt_embeds.dtype)
                    .repeat(repeat_factor, 1),
                }
        ...
        if not output_type == "latent":
            ...
            # 稳定修复（更干净）：直接用 fp32 做 VAE decode，避免某些模型/算子组合在 decoder 内触发 Half/Float mismatch
            #（例如 group_norm 里 input=float32 但 weight=float16）。
            restore_dtype = None
            if (not needs_upcasting) and self.vae.dtype in (torch.float16, torch.bfloat16):
                restore_dtype = self.vae.dtype

            # decode 统一走 fp32（与上游 diffusers 的“VAE decode 用 fp32 更稳”思路一致）
            self.vae.to(dtype=torch.float32)
            latents_vae = latents.to(dtype=torch.float32) / self.vae.config.scaling_factor
            image = self.vae.decode(latents_vae, return_dict=False)[0]

            # 若不是 force_upcast 场景，恢复原 dtype，避免后续显存/性能异常
            if restore_dtype is not None:
                self.vae.to(dtype=restore_dtype)
