...
def load_model_torch(model, ckpt_path):
    if distributed.is_rank0():
        # NOTE:
        # Upstream used `map_location="cuda"` for fast startup, but with newer torch + `assign=True`,
        # a checkpoint tensor on device "cuda" (no index) cannot be assigned into a param on "cuda:0".
        # We map explicitly to the current CUDA device to avoid "cuda" vs "cuda:0" mismatch.
        if torch.cuda.is_available():
            dev_idx = int(torch.cuda.current_device())
            map_loc = torch.device("cuda", dev_idx)
        else:
        ...
        # Extra safety: normalize any stray "cuda" (no index) tensors to the explicit device.
        try:
            if isinstance(ckpt, dict) and torch.cuda.is_available():
                for k, v in list(ckpt.items()):
                    if isinstance(v, torch.Tensor):
                        try:
                            if str(v.device) == "cuda":
                                ckpt[k] = v.to(device=map_loc)
                        except Exception:
                            pass
        except Exception:
            pass
