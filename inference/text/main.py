...
from text.inferrer import TextInferrer  # noqa: E402
...
async def main() -> None:
    if len(sys.argv) < 2:
        logger.error("Usage: python inference/text/main.py <service_id>")
        sys.exit(1)
    ...
    if service_type != "text":
        ...
        logger.error(
            ...
            f"'{config_path}', but this entrypoint only supports 'text'. "
            f"Recommended command: {recommended_cmd}"
        )
        sys.exit(1)

    inferrer = TextInferrer(service_id=service_id)
    try:
        await inferrer.run()
