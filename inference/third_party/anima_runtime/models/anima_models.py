...
# Anima Model Architecture (vendored for inference runtime)
...
from .. import custom_offloading_utils, attention
...
class GPT2FeedForward(nn.Module):
    def __init__(self, d_model: int, d_ff: int, *, initialize_weights: bool = False) -> None:
        super().__init__()
...
class Attention(nn.Module):
    ...
    def compute_qkv(self, x: torch.Tensor, context: Optional[torch.Tensor] = None, rope_emb: Optional[torch.Tensor] = None) -> tuple:
        q = self.q_proj(x)
...
class VideoRopePosition3DEmb(VideoPositionEmb):
    def __init__(
        ...
        **kwargs,
        ...
        self.register_buffer("dim_spatial_range", torch.arange(0, dim_h, 2)[: (dim_h // 2)].float() / dim_h, persistent=True)
        self.register_buffer("dim_temporal_range", torch.arange(0, dim_t, 2)[: (dim_t // 2)].float() / dim_t, persistent=True)
        self._dim_h = dim_h
        self._dim_t = dim_t
...
class TimestepEmbedding(nn.Module):
    def __init__(self, in_features: int, out_features: int, use_adaln_lora: bool = False, *, initialize_weights: bool = False):
        super().__init__()
        ...
        self.linear_2 = nn.Linear(out_features, 3 * out_features, bias=False) if use_adaln_lora else nn.Linear(out_features, out_features, bias=False)
        if initialize_weights:
            self.init_weights()
...
class Block(nn.Module):
    def __init__(self, x_dim: int, context_dim: int, num_heads: int, mlp_ratio: float = 4.0, use_adaln_lora: bool = False, adaln_lora_dim: int = 256):
        ...
        self.self_attn = Attention(x_dim, None, num_heads, x_dim // num_heads, qkv_format="bshd", initialize_weights=False)
        self.layer_norm_cross_attn = nn.LayerNorm(x_dim, elementwise_affine=False, eps=1e-6)
        self.cross_attn = Attention(x_dim, context_dim, num_heads, x_dim // num_heads, qkv_format="bshd", initialize_weights=False)
        self.layer_norm_mlp = nn.LayerNorm(x_dim, elementwise_affine=False, eps=1e-6)
        self.mlp = GPT2FeedForward(x_dim, int(x_dim * mlp_ratio), initialize_weights=False)
        self.use_adaln_lora = use_adaln_lora
        if self.use_adaln_lora:
    ...
    def _forward(
        ...
        normed = _adaln(x_B_T_H_W_D, self.layer_norm_self_attn, scale_self, shift_self)
        result = rearrange(
        ...
        x_B_T_H_W_D = x_B_T_H_W_D + gate_self * result

        normed = _adaln(x_B_T_H_W_D, self.layer_norm_cross_attn, scale_cross, shift_cross)
        result = rearrange(
        ...
        x_B_T_H_W_D = result * gate_cross + x_B_T_H_W_D
...
class Anima(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        in_channels_eff = self.in_channels + 1 if self.concat_padding_mask else self.in_channels
        self.x_embedder = PatchEmbed(self.patch_spatial, self.patch_temporal, in_channels=in_channels_eff, out_channels=self.model_channels)
        ...
        self.t_embedder = nn.Sequential(
            ...
            TimestepEmbedding(model_channels, model_channels, use_adaln_lora=use_adaln_lora, initialize_weights=initialize_weights),
        )

        if self.use_llm_adapter:
            self.llm_adapter = LLMAdapter(source_dim=1024, target_dim=1024, model_dim=1024, num_layers=6, self_attn=True)
    ...
    def prepare_embedded_sequence(self, x_B_C_T_H_W: torch.Tensor, fps: Optional[torch.Tensor] = None, padding_mask: Optional[torch.Tensor] = None):
        if self.concat_padding_mask:
            # Avoid torchvision dependency: nearest resize via interpolate.
            if padding_mask is None:
                padding_mask = torch.zeros(x_B_C_T_H_W.shape[0], 1, x_B_C_T_H_W.shape[-2], x_B_C_T_H_W.shape[-1], device=x_B_C_T_H_W.device, dtype=x_B_C_T_H_W.dtype)
            if padding_mask.shape[-2:] != x_B_C_T_H_W.shape[-2:]:
                pm = padding_mask.unsqueeze(1)  # [B,1,1,H,W] or [B,1,H,W]->[B,1,1,H,W]
                if pm.dim() == 5:
                    pm2d = pm[:, :, 0]
                else:
                    pm2d = pm
                pm2d = F.interpolate(pm2d, size=list(x_B_C_T_H_W.shape[-2:]), mode="nearest")
                padding_mask = pm2d.squeeze(1)

            x_B_C_T_H_W = torch.cat([x_B_C_T_H_W, padding_mask.unsqueeze(1).repeat(1, 1, x_B_C_T_H_W.shape[2], 1, 1)], dim=1)

        x_B_T_H_W_D = self.x_embedder(x_B_C_T_H_W)
        rope = self.pos_embedder(x_B_T_H_W_D, fps=fps)
        return x_B_T_H_W_D, rope
    ...
    def forward_mini_train_dit(
        ...
        x_B_T_H_W_D, rope_emb_L_1_1_D = self.prepare_embedded_sequence(x_B_C_T_H_W, fps=fps, padding_mask=padding_mask)
        ...
        for block_idx, block in enumerate(self.blocks):
            ...
            x_B_T_H_W_D = block(x_B_T_H_W_D, t_embedding_B_T_D, crossattn_emb, attn_params, rope_emb_L_1_1_D, adaln_lora_B_T_3D)
            if self.blocks_to_swap:
    ...
    def forward(
        ...
        **kwargs,
        ...
        return self.forward_mini_train_dit(
            ...
            source_attention_mask=source_attention_mask,
            t5_input_ids=target_input_ids,
            t5_attn_mask=target_attention_mask,
        )
...
class LLMAdapterTransformerBlock(nn.Module):
    def __init__(self, source_dim, model_dim, num_heads=16, mlp_ratio=4.0, self_attn=False, layer_norm=False):
        ...
        if self.has_self_attn:
            ...
            self.self_attn = LLMAdapterAttention(model_dim, model_dim, n_heads=num_heads, head_dim=model_dim // num_heads)
        self.norm_cross_attn = nn.LayerNorm(model_dim) if layer_norm else LLMAdapterRMSNorm(model_dim)
        self.cross_attn = LLMAdapterAttention(model_dim, source_dim, n_heads=num_heads, head_dim=model_dim // num_heads)
    ...
    def forward(self, x, context, target_attention_mask=None, source_attention_mask=None, position_embeddings=None, position_embeddings_context=None):
        if self.has_self_attn:
            ...
            x = x + self.self_attn(normed, mask=target_attention_mask, position_embeddings=position_embeddings, position_embeddings_context=position_embeddings)
