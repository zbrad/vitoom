...
def create_musetalk_human(file, avatar_id):
    ...
    # === 上游 bug 修复：placeholder 帧需要被填充而不是被跳过 ===
    # 上游原始流程：
    #   1) 计算 latent 时遇到 (0,0,0,0) placeholder 就 continue 跳过；
    #   2) 但 mask/coord 列表仍保留这些 placeholder 帧；
    #   3) 周期化后 latents.pt 比 coords.pkl/mask 短，长度错位；
    #   4) 推理时 sidecar 用相同 idx 取 (0,0,0,0) bbox 去 paste_back，
    #      触发 "could not broadcast input array from shape (...) into shape (...,0,3)"
    #      并整段卡帧。
    # 修复策略：用前向(以及必要时后向)填充把 placeholder 替换成最近的有效 bbox，
    # 保证四个 cycle 列表长度一致 + 没有零宽/零高 bbox。
    coord_placeholder = (0.0, 0.0, 0.0, 0.0)
    valid_bboxes = [b for b in coord_list if b != coord_placeholder]
    if not valid_bboxes:
        raise RuntimeError(
            "No face detected in any frame. Please supply a clearer video, "
            "or lower SFD filter_threshold in face_detection/detection/sfd/sfd_detector.py."
        )

    placeholder_count = len(coord_list) - len(valid_bboxes)
    if placeholder_count > 0:
        print(f"[genavatar] {placeholder_count}/{len(coord_list)} frames have no face detected; "
              f"filling them with neighbor bbox to keep cycle lists aligned.")

    # leading placeholders 用首个有效 bbox 兜底；其余按前向填充
    last_valid = valid_bboxes[0]
    for i, b in enumerate(coord_list):
        if b != coord_placeholder:
            last_valid = b
        else:
            coord_list[i] = last_valid
