# Licensed under Apache License 2.0; derived from
# https://github.com/zhexiao/mtef-go (see ./NOTICE.md and ./LICENSE).
# Local modifications: olefile-based OLE reader, debug-print removal,
# tightened public surface. No MTEF semantic changes.
...
from ._chars import Chars, SpecialChar
...
class _Helper:
    """Minimal subset of upstream's ``ole_util.helper.Helper`` used by the
    MTEF body parser. All inputs are little-endian byte strings."""

    @staticmethod
    def bytes2int(data: Optional[bytes]) -> int:
        if not data:
            return 0
...
class MTEF:
    ...
    @classmethod
    def parse_body(cls, body: bytes) -> "MTEF":
        """Parse a raw MTEF body (the bytes that follow the 28-byte
        ``EQNOLEFILEHDR`` inside an OLE ``Equation Native`` stream).

        Replaces upstream ``Open(reader)`` which used a hand-rolled OLE
        reader; OLE I/O is now handled by ``olefile`` in
        ``mathtype_ole_to_latex`` (see ``__init__.py``)."""
        eqn = cls()
